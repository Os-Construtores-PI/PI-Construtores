...
namespace LineworkLite.FreeOutline
{
    ...
    public class FreeOutline : ScriptableRendererFeature
    {
        private class FreeOutlinePass : ScriptableRenderPass
        {
            ...
            public bool Setup(
                ref FreeOutlineSettings freeOutlineSettings,
                ref Material maskMaterial,
                ref Material outlineMaterial,
                ref Material clearMaterial
            )
            {
                ...
                foreach (var outline in settings.Outlines)
                {
                    ...
                    switch (outline.maskingStrategy)
                    {
                        ...
                        case MaskingStrategy.CullFrontFaces:
                            material.SetFloat(
                                CommonShaderPropertyId.CullMode,
                    ...
                    material.SetColor(
                        ...
                        outline.occlusion == Occlusion.WhenOccluded
                            ? outline.color
                            : outline.occludedColor
                    );
                    material.SetFloat(ShaderPropertyId.OutlineWidth, outline.width);

                    // Scale with resolution.
                    if (outline.scaleWithResolution)
                        material.EnableKeyword(ShaderFeature.ScaleWithResolution);
                    else
                        material.DisableKeyword(ShaderFeature.ScaleWithResolution);
                    switch (outline.referenceResolution)
                    {
                        ...
                        case Resolution.Custom:
                            material.SetFloat(
                                ShaderPropertyId.ReferenceResolution,
                                outline.customResolution
                            );
                            break;
                    }
                    ...
                    else
                    {
                        ...
                        material.SetFloat(
                            ShaderPropertyId.MinOutlineWidth,
                            outline.minWidth * 0.015f
                        );
                    }
                    if (outline.enableOcclusion)
                        material.EnableKeyword(ShaderFeature.Occlusion);
                    else
                        material.DisableKeyword(ShaderFeature.Occlusion);
                    if (outline.scaling == Scaling.ScaleWithDistance)
                        material.EnableKeyword(ShaderFeature.ScaleWithDistance);
                    else
                        material.DisableKeyword(ShaderFeature.ScaleWithDistance);
                    switch (outline.occlusion)
                    {
                        case Occlusion.Always:
                            material.SetFloat(
                                CommonShaderPropertyId.ZTest,
            ...
            )
            {
                ...
                foreach (var outline in settings.Outlines)
                {
                    var drawingSettings = RenderingUtils.CreateDrawingSettings(
                        RenderUtils.DefaultShaderTagIds,
                        renderingData,
                        cameraData,
                        lightData,
                        sortingCriteria
                ...
                foreach (var outline in settings.Outlines)
                {
                    ...
                    switch (outline.materialType)
                    {
                        case MaterialType.Basic:
                            ...
                            drawingSettings.overrideMaterialPassIndex = (int)
                                outline.extrusionMethod;
            ...
            )
            {
                ...
                using (new ProfilingScope(outlineCmd, outlineSampler))
                {
                    CoreUtils.SetRenderTarget(
                        ...
                        cameraDepthRTHandle
                    ); // if using cameraColorRTHandle this does not render in scene view when rendering after post processing with post processing enabled
                ...
                using (new ProfilingScope(clearStencilCmd, outlineSampler))
                {
                    ...
                    clearStencilCmd.DrawProcedural(
                        Matrix4x4.identity,
                        clear,
                        0,
...
#pragma warning restore 618, 672
        ...
        {
            if (settings == null || freeOutlinePass == null)
                return;
            ...
            {
                Debug.LogWarning(
                    "Not all required materials could be created. Free Outline will not render."
                );
                return;
            }

            var render = freeOutlinePass.Setup(
                ref settings,
                ref maskMaterial,
                ref outlineMaterial,
                ref clearMaterial
