...
namespace CartoonFX
{
    public partial class CFXR_Effect : MonoBehaviour
    {
        ...
        public class CameraShake
        {
            ...
            Dictionary<Camera, Vector3> camerasPreRenderPosition =
                new Dictionary<Camera, Vector3>();
            Vector3 shakeVector;
            float delaysTimer;
            ...
            public void animate(float time)
            {
                ...
                if (time < totalDuration)
                {
                    ...
                    var shakeVec =
                        Vector3.Scale(randomVec, shakeStrength) * (Random.value > 0.5f ? -1 : 1);
                    shakeVector =
                        shakeVec * shakeCurve.Evaluate(delta) * GLOBAL_CAMERA_SHAKE_MULTIPLIER;
                }
                else if (isShaking)
                {
                    StopShake();
