    ...
    public class CFXR_Demo : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (isURP)
            {
                ...
                if (camUrpData == null)
                    camUrpData =
                        demoCamera.gameObject.AddComponent<UniversalAdditionalCameraData>();
        ...
        static class ButtonsPressed
        {
            ...
            internal static Vector2 MousePosition =>
                Mouse.current != null ? Mouse.current.position.value : Vector2.zero;
            internal static float MouseScrollY =>
                Mouse.current != null ? Mouse.current.scroll.value.y : 0;
#else
        ...
        void Update()
        {
            ...
            if (ButtonsPressed.Mouse0)
            {
                var ray = demoCamera
                    .GetComponent<Camera>()
                    .ScreenPointToRay(ButtonsPressed.MousePosition);
            ...
            if (scroll != 0f)
            {
                demoCamera.transform.Translate(
                    Vector3.forward * (scroll < 0f ? -1f : 1f) * zoomFactor,
                    Space.Self
                );
            }
        }
