        ...
        public static GUIStyle CloseCrossButton
        {
            get
            {
                if (_closeCrossButton == null)
                {
                    ...
                    string guiSkinPath = AssetDatabase.GUIDToAssetPath(
                        "02d396fa782e5d7438e231ea9f8be23c"
                    );
                    var gs = AssetDatabase.LoadAssetAtPath<GUISkin>(guiSkinPath);
                    if (gs != null)
                    {
                        _closeCrossButton = System.Array.Find<GUIStyle>(
                            gs.customStyles,
                            x => x.name == "CloseCrossButton"
                        );
                    }
        ...
        public static void HelpBoxRichText(Rect position, string message, MessageType msgType)
        {
            ...
            switch (msgType)
            {
                ...
                case MessageType.Info:
                    icon =
                        infoIcon
                        ?? (infoIcon = EditorGUIUtility.Load("console.infoicon") as Texture2D);
                    break;
