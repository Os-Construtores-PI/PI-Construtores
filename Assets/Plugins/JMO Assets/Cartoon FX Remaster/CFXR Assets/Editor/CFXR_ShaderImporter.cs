...
namespace CartoonFX
{
    namespace CustomShaderImporter
    {
        static class Utils
        {
            public static bool IsUsingURP()
            {
                ...
                return renderPipeline != null
                    && renderPipeline.GetType().Name.Contains("Universal");
            }
        }
        ...
        public class CFXR_ShaderImporter : ScriptedImporter
        {
            ...
            static bool CompareWithOperator(
                int value1,
                int value2,
            ...
            )
            {
                switch (comparisonOperator)
                {
                    ...
                    default:
                        throw new Exception(
                            "Invalid comparison operator value: " + comparisonOperator
                        );
                }
            }
            ...
            public override void OnImportAsset(AssetImportContext context)
            {
                ...
                switch (renderPipelineDetection)
                {
                    ...
                    case RenderPipeline.Auto:
                    {
                        ...
                        detectedRenderPipeline = isUsingURP
                ...
                {
                    bool excludeThisLine =
                        excludeCurrentLines.Count > 0 && excludeCurrentLines.Peek();
                    ...
                    if (StartsOrEndWithSpecialTag(line))
                    {
                        ...
                        else if (line.StartsWith("/*** URP_VERSION "))
                        {
                            ...
                            string version = subline.Substring(
                                spaceIndex,
                                subline.LastIndexOf(' ') - spaceIndex
                            );
                            ...
                            bool isCorrectURP = CompareWithOperator(
                                URP_VERSION,
                                compVersion,
                                compOp
                            );
                        ...
                        else if (excludeThisLine && line.StartsWith("/*** END"))
                        {
                            ...
                            !excludeThisLine && line.StartsWith("/*** #define URP_VERSION ***/")
                        )
                        {
                ...
                int idx =
                    shaderSourceCode.IndexOf("Shader \"", StringComparison.InvariantCulture) + 8;
                ...
                {
                    string[] shaderSourceLines = shaderSourceCode.Split(
                        new[] { '\n' },
                        StringSplitOptions.None
                    );
                    var errors = ShaderUtil.GetShaderMessages(shader);
                    shaderErrors = Array.ConvertAll(
                        errors,
                        err => $"{err.message} (line {err.line})"
                    );
                    foreach (ShaderMessage error in errors)
                    {
                        string message =
                            error.line <= 0
                                ? string.Format(
                                    "Shader Error in '{0}' (in file '{2}')\nError: {1}\n",
                                    shaderName,
                                    error.message,
                                    error.file
                                )
                                : string.Format(
                                    "Shader Error in '{0}' (line {2} in file '{3}')\nError: {1}\nLine: {4}\n",
                                    shaderName,
                                    error.message,
                                    error.line,
                                    error.file,
                ...
                variantCount = 0;
                variantCountUsed = 0;
                MethodInfo getVariantCountReflection = typeof(ShaderUtil).GetMethod(
                    "GetVariantCount",
                ...
                if (getVariantCountReflection != null)
                {
                    try
                    {
                        object result = getVariantCountReflection.Invoke(
                            null,
                            new object[] { shader, false }
                        );
                        variantCount = (ulong)result;
                        result = getVariantCountReflection.Invoke(
                            null,
                            new object[] { shader, true }
                        );
        ...
        namespace Inspector
        {
            ...
            public class TCP2ShaderImporter_Editor : Editor
            {
                ...
                public override void OnInspectorGUI()
                {
                    ...
                    CFXR_ShaderImporter.RenderPipeline detection = (
                        (CFXR_ShaderImporter)target
                    ).renderPipelineDetection;
                    ...
                    {
                        string variantsCount = multipleValues
                            ? "-"
                            : FormatCount(Importer.variantCount);
                        string variantsCountUsed = multipleValues
                            ? "-"
                            : FormatCount(Importer.variantCountUsed);
                        variantsText =
                            $"\nVariants (currently used): <b>{variantsCountUsed}</b>\nVariants (including unused): <b>{variantsCount}</b>";
                    }
                    string strippedLinesCount = multipleValues
                        ? "-"
                        : Importer.strippedLinesCount.ToString();
                    ...
                    if (
                        detection == CFXR_ShaderImporter.RenderPipeline.Auto
                    ...
                    EditorGUILayout.PropertyField(
                        serializedObject.FindProperty(
                            nameof(CFXR_ShaderImporter.renderPipelineDetection)
                        )
                    );
