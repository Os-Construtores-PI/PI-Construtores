...
namespace CartoonFX
{
    public class CFXR_ParticleTextFontAsset : ScriptableObject
    {
        ...
        {
            bool valid =
                !string.IsNullOrEmpty(CharSequence)
                && CharSprites != null
                && CharSprites.Length == CharSequence.Length
                && CharKerningOffsets != null
                && CharKerningOffsets.Length == CharSprites.Length;

            if (!valid)
            {
                Debug.LogError(
                    string.Format("Invalid ParticleTextFontAsset: '{0}'\n", this.name),
                    this
                );
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Import Kerning"))
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    List<CFXR_ParticleTextFontAsset.Kerning> kerningList =
                        new List<CFXR_ParticleTextFontAsset.Kerning>(ptfa.CharKerningOffsets);
