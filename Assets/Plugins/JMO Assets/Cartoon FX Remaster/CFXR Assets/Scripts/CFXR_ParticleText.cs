...
namespace CartoonFX
{
    ...
    public class CFXR_ParticleText : MonoBehaviour
    {
        ...
        [SerializeField]
        bool cumulativeDelay = false;

        [Range(0f, 2f)]
        [SerializeField]
        float compensateLifetime = 0;
        ...
        [SerializeField]
        float sortingFudgeOffset = 0.1f;
#pragma warning disable 0649
        [SerializeField]
        CFXR_ParticleTextFontAsset font;
#pragma warning restore 0649

#if UNITY_EDITOR
        [HideInInspector]
        [SerializeField]
        bool autoUpdateEditor = true;
        ...
        void InitializeFirstParticle()
        {
            ...
            var ps = isDynamic
                ? this.transform.GetChild(0).GetComponent<ParticleSystem>()
        ...
        )
        {
            ...
            if (newText != null)
            {
                ...
                foreach (char c in newText)
                {
                    ...
                    {
                        throw new System.Exception(
                            "[CFXR_ParticleText] Invalid character supplied for the dynamic text: '"
                                + c
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        float charWidth =
                            sprite.rect.width
                            + font.CharKerningOffsets[index].post
            ...
            if (!Application.isPlaying)
            {
                ...
                while (this.transform.childCount > 1)
                {
                    Object.DestroyImmediate(
                        this.transform.GetChild(this.transform.childCount - 1).gameObject
                    );
                    overflow++;
                    if (overflow > 1000)
                    {
                ...
                for (int i = 0; i < text.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (cumulativeDelay)
                        {
                            ...
                            mainModule.startLifetime = Mathf.LerpUnclamped(
                                baseLifetime,
                                baseLifetime + (delay * (text.Length - i)),
                                compensateLifetime / lifetimeMultiplier
                            );
                        }
                        else
                        {
                        ...
                        mainModule.startLifetime =
                            mainModule.startLifetime.constant * lifetimeMultiplier;
                        ...
                        particleRenderer.pivot = new Vector3(
                            basePivot.x + position,
                            basePivot.y,
                            basePivot.z
                        );
    ...
    public class ParticleTextEditor : Editor
    {
        ...
        GUIContent GUIContent_AutoUpdateToggle = new GUIContent(
            "Auto-update",
            "Automatically regenerate the text when a property is changed."
        );
        GUIContent GUIContent_UpdateTextButton = new GUIContent(
            " Update Text ",
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                autoUpdateBool.boolValue = GUILayout.Toggle(
                    autoUpdateBool.boolValue,
                    GUIContent_AutoUpdateToggle,
                    GUILayout.Height(30)
                );
