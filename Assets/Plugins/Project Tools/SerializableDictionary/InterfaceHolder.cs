        ...
        where T : class
    {
        ...
        public T Value
        {
            get
            {
                ...
                {
                    Debug.LogError(
                        $"value cannot be cast to {typeof(T)}. It is of type {value.GetType()}"
                    );
                }
    ...
    public class InterfaceHolderDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (
                    newValue == null
                    || newValue.GetComponent(fieldInfo.FieldType.GenericTypeArguments[0]) != null
                )
                {
                ...
                {
                    Debug.LogWarning(
                        $"Assigned object must implement interface {fieldInfo.FieldType.GenericTypeArguments[0].Name}"
                    );
                }
            }
